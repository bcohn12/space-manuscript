...
\textbf\newline{The high-dimensional structure of feasible muscle activations reconciles alternative approaches to motor control } % Please use "title case" (capitalize all terms in the title except conjunctions, prepositions, and articles).
% multimuscle
}
...
%How the nervous system selects a particular solution from within this infinite set of options is hotly debated. In general, current thinking favors three theories of motor control: that the nervous system combines low-dimensional synergies, learns probabilistic representations of valid muscle activations, or optimizes physiologically-tenable cost functions \cite{valero-cuevas2015fundamentals}(Figure \ref{fig:overview}(g, h, i), respectively).
%In this work we characterize the set of all feasible muscle activations in their \emph{native high-dimensional space} in sufficient detail to reconcile these three alternative approaches to motor control. 
%Well-posed underdetermined problems---like muscle redundancy---have well-defined solution spaces embedded in the native high-dimensional space of all variables \cite{Chvatal1983Linear}.
...
%This detailed view of how the structure of feasible motor actions affects fitness landscapes of alternative cost functions  suggests that habitual, memory-based Bayesian strategies are the more biologically plausible approach to sensorimotor control. 

%The number of muscles across a limb, $n$, defines an n-dimensional space of feasible activations where each muscle can be activated between 0 and 1. The subset of this n-dimensional positive cube that is able to produce a given sub-maximal static force is called the feasible activation set for the task; if it exists, it is a convex polytope defined by the constraints that arise from the mechanical definition of the task and the properties of the limb. Even for 'simple' models, those with a reasonable number of muscles performing a well constrained task \cite{Valero-Cuevas1998Large,valero-cuevas2015fundamentals,sohn2013cat_bounding_box,Chao1978Graphical,spoor1983balancing}, interpreting the set of feasible activations in high dimensions is difficult. \cite{bellman1957dynamic,bellman2015adaptive}. 

%Consider a one-link limb with 3 muscles producing a static force. In this case a simple example of a feasible activation set would be represented by a plane embedded in the positive 3-dimensional cube, Fig. \ref{fig:hitruncube}. There are an infinite number of solutions to the task (i.e.\, points in the plane)---and any learning, optimization, or performance of the task involves finding, exploring, and exploiting the landscape of that plane. In spite of the model's simplicity, one can already see that describing and understanding the properties of this convex polygon for the purposes of gaining insight into neural control can be challenging. For example, describing the constraint equations or vertices that define its boundaries it do not speak to the many coordination strategies in its interior. Finding its bounding box can overestimate its size (while a bounding box of a line segment has volume, the line has no volume). And principal component analysis of points within the polygon provides a non-unique basis function with information equivalent to that already available from the constraint equations and vertices. Defining the structure of this space may be less useful then sampling it in a way that provides histogram densities of possible muscle activations, and parallel coordinates of cross-muscle relationships.
...
% \begin{tabular}{@{}ll@{}}
% \toprule
% \textbf{Abbreviation} & \textbf{Cost function} & \textbf{Name}  \\
%  \\
% $L_1$            & $\sum_{i=1}^n a_i$         & Sum of activations                            \\
%  \\
% $L_2$            & $\sqrt{\uprule\sum_{i=1}^n a_i^2}$   & Euclidean norm of activations                            \\
%  \\
% %$L_3$            & $\sqrt[3]{\uprule\sum_{i=1}^n a_i^3}$                          \\
%  %\\
% $L_1^w$            & $\sum_{i=1}^n a_i F_{0i}$     & Sum of strength-weighted activations                          \\
%  \\
% $L_2^w$            & $\sqrt{\uprule\sum_{i=1}^n (a_i F_{0i})^2}$  & Euclidean norm of strength-weighted activations                \\
%  \\
% %$L_3^w$            & $\sqrt[3]{\uprule\sum_{i=1}^n (a_i F_{0i})^3}$          \\
% %  \\
% \end{tabular}
% \caption{Linear and quadratic cost functions. The variables $a_i$ and $F_{0i}$ represent the activation of the $i^{th}$ muscle in a given muscle activation pattern, and the maximal strength of each muscle \cite{Prilutsky2000Muscle,crowninshield1981physiologically}. Maximal muscle strengths are approximated by the multiplying  each muscle's physiological cross-sectional area, in $cm^2$, by the maximal active muscle stress of mammalian muscle, $35~N/{cm^2}$ \cite{Zajac1993Muscle}
% .}
...
% [[don't think we need this]]Next, we see that fixing the acivation of a muscle to a single value throughout the entire force progression, DI and PI would fail; their bounding boxes of tasks below $\alpha=0.4$ do not include the unique solution at $\alpha=1.0$.
...
%Since $\alpha \textbf{f}_{\max} = \alpha H \textbf{a}^*$, $\alpha \textbf{a}^*$ is a solution of the feasible activation set at $\alpha$-fraction of the maximal force.
